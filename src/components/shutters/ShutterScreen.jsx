...
export default function KleidSysLoader() {
  ...
  const logoRef = useRef(null); // Changed to logoRef for SVG
  ...
  // Initialize SVG for drawing animation
  useEffect(() => {
    ...
    paths.forEach((path) => {
      ...
      // Force visibility for length calculation
      path.style.display = "block";
    });

    // Calculate and setup dasharray for each path
    paths.forEach((path, index) => {
      const length = path.getTotalLength();
      
      if (length > 0) {
        console.log(`Path ${index}: Length = ${length.toFixed(0)}`);
        
        // Store length in dataset
        path.dataset.pathLength = length;
        
        // Setup initial dasharray (hidden state)
  ...
  useEffect(() => {
    ...
    // PHASE 1: Logo Container Hanging (0-0.8s)
    ...
    // PHASE 2: Bounce/settle (0.8-1.4s)
    ...
    // PHASE 3: Floating animation (1.4s+)
    tl.to(logoContainerRef.current, {
      y: "-=15",
      duration:1,
      ease: "sine.inOut",
    ...
    // PHASE 4: SVG Drawing Animation (2.0s - start after bounce settles)
    const drawingTimeline = gsap.timeline({ delay:0.1});
    
    if (logoRef.current) {
      const svg = logoRef.current;
      const paths = Array.from(svg.querySelectorAll("path"));
      
      console.log("Starting SVG path drawing animation");
      
      // Animate each path drawing with stagger
      paths.forEach((path, index) => {
        const length = parseFloat(path.dataset.pathLength || 0);
        
        if (length > 0) {
          // Drawing animation for this path
          drawingTimeline.fromTo(
            ...
            },
            {
              ...
              duration: 0.1,
              ease: "power2.out",
              // Stagger by 0.15s between paths
              delay: index * 0.1,
            }
          ).to(
            path,
            {
              // Add a slight bounce after drawing
              scale: 1,
              yoyo: true,
              repeat: 1,
              duration: 0.1,
      ...
      // PHASE 5: Color Fill Animation (4.0s - after all paths drawn)
      ...
      // Fill primary paths (dark blue)
      drawingTimeline.to(primaryPaths, {
        fill: BRAND_COLORS.primary,
        stroke: "none", // Remove stroke after filling
        duration: 0.1,
        ease: "back.out(1.7)",
        stagger: {
          amount: 0.1,
      ...
      // Fill accent paths (bright blue) - slightly delayed
      if (accentPaths.length > 0) {
        drawingTimeline.to(accentPaths, {
          fill: BRAND_COLORS.secondary,
          stroke: "none",
          scale: 1.1, // Slight scale up for accent
          duration: 0.1,
          ease: "back.out(2)",
          delay:2.2,
        }, "fill-accent");
      ...
      // PHASE 6: SVG Glow Effect (5.5s)
      drawingTimeline.to(svg, {
        filter: `drop-shadow(0 0 25px ${BRAND_COLORS.secondary}) drop-shadow(0 0 15px ${BRAND_COLORS.primary})`,
        scale: 1,
        duration: 0.1,
        ease: "power2.out",
        delay:0.1,
    ...
    // PHASE 7: Text Reveal (5.8s - after SVG animation completes)
    const textTimer = setTimeout(() => {
      console.log("Showing text");
      setShowText(true);
    }, 5800);

    // PHASE 8: Shutter Effect (8.0s - give time for text to appear)
    const shutterTimer = setTimeout(() => {
      ...
      gsap.to(overlayRef.current, {
        ...
        onComplete: () => {
          setShowContent(true);
          // Reset overlay scale
          gsap.to(overlayRef.current, { scale: 1, duration: 0.1 });
        },
      });
    }, 90000000);
  ...
  // Text animation with custom font
  useEffect(() => {
    ...
    gsap.fromTo(
      ...
      },
      {
        ...
        ease: "back.out(2)",
        onComplete: () => {
          // Add glow effect with brand colors
          gsap.to(text, {
      ...
      >
        <style>{`
          * {
          ...
          /* Custom font for Kleidsys text - you can replace with your preferred font */
          ...
          .logo-svg path {
            ...
            will-change: stroke-dashoffset, opacity, fill, stroke;
          }
          ...
          .hero-image {
            ...
            bottom: 0;
            background: 
              radial-gradient(circle at 30% 70%, ${BRAND_COLORS.secondary}40, transparent 60%),
              radial-gradient(circle at 70% 20%, ${BRAND_COLORS.accent}30, transparent 60%);
            opacity: 0.2;
            z-index: 1;
          }
          ...
          .hero-title {
            ...
            margin-bottom: 2rem;
            opacity: 0;
            transform: translateY(100px) rotateX(-20deg) scale(0.7);
            text-shadow: none;
  ...
  //       <div ref={logoContainerRef} className="logo-container gap">
